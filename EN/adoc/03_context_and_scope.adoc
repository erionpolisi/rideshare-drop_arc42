...
own (see constraints TC3, TC4 and OC6).
...
and exposes a REST API over HTTPS. It uses Firebase as a backing service
- Firestore for ride, profile and location data, and Firebase Cloud
Messaging for push notifications; clients receive real-time changes such
as live tracking through Firestore listeners. Maps, routing and payment
are reached through external providers over the Internet.
...
| Ride data and real-time tracking
| Firebase database (Firestore) over the Internet (HTTPS / WebSocket)
