...
Flow::
=======
. *Rides* asks *Locations* for the available drivers.
. *Locations* queries the *Map & Routing Provider*,
...
. *Rides* persists the ride in the *Database*.
. *Rides* triggers location sharing through *Locations*, which shares the Passenger's location with the *Driver*.
. *Rides* asks *Notifications* to confirm the booking to the passenger.
 
=== Scenario 2 – Ride Completion and Payment

image::../images/06_payment.png[Ride Completion and Payment Runtime View]
...
. *Rides* asks *Payments* to initiate the payment; *Payments* calls the
  *PaymentAPI* microservice (Payment Processor) and receives confirmation that the payment has been processed.
. *Payments* stores the transaction in the *Database* and returns confirmation to *Rides*.
